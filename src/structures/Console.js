class Console {
	constructor(client) {
		...
		console.log = (color, ...args) => {
			if (!this[color]) { args = [color, ...args]; (color = 'blue'); }

			const moment = require('moment')();
			const time = require('chalk')`{bold.hex('${this[color][0]}') [{underline ${new Date().toLocaleDateString()}} - {underline ${moment.format('hh:mm:ss A')}}]} {hex('${this[color][1]}') |}`;
		...
		console.error = (...args) => {
			const moment = require('moment')();
			const time = require('chalk')`{bold.hex('${this.red[0]}') [{underline ${new Date().toLocaleDateString()}} - {underline ${moment.format('hh:mm:ss A')}}]} {hex('${this.red[1]}') |}`;
